...
def dispatch_tool(tool_name: str, tool_input: dict) -> str:
    ...
    try:
        if tool_name == "future_value":
            ...
            cf = [tuple(x) for x in tool_input["cashflows"]]
            i = tool_input["i"]
            val_date = tool_input.get("valuation_date", 0.0)
            result = equation_of_value(cf, i, val_date)
            return json.dumps({"net_value_at_valuation_date": round(result, 6)})

        elif tool_name == "solve_unknown_payment":
            cf = [tuple(x) for x in tool_input["known_cashflows"]]
